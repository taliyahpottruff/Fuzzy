...
class InfractionAdmin(Fuzzy.Cog):
    ...
    async def pardon(self,
        ...
        for infraction_id in infraction_ids:
            infraction: Infraction = ctx.db.infractions.find_by_id(infraction_id, ctx.guild.id)
    ...
    async def reason(self,
        ...
        log if if has been `$pfxpublish`ed.
        ...
        for infraction in all_infractions:
            ...
            if infraction.infraction_type == InfractionType.BAN and infraction.published_ban is not None:
                message: discord.Message = ctx.author.fetch_message(infraction.published_ban.message_id)
    ...
    async def publish(self,
        ...
        If ban has already been posted you can use $pfxreason to update it.
        ...
        if all_errors or all_non_bans:
            msg = ""
            if all_errors:
                msg += "Could not find infractions with IDs: " + " ".join(all_errors) + "\n"
            if all_non_bans:
                msg += "These infractions were not bans and therefore not published: " + " ".join(all_non_bans) + "\n"
            await ctx.reply(msg, color=ctx.Color.I_GUESS)
    ...
    def create_ban_embed(infraction: Infraction):
        return discord.Embed(description=f"**Date:** {infraction.infraction_on.strftime('%Y-%m-%d')}\n"
